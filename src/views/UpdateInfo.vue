<template>
  ...
  <el-descriptions title="用户信息" :column="3" border>
    ...
    ><el-input v-model="email"/></el-descriptions-item
    >
    <el-descriptions-item label="昵称" label-align="right" align="center">
      <el-input v-model="nickName"/>
    </el-descriptions-item>
  </el-descriptions>
    <button @click=modUserinfo>修改</button>
  </div>
</template>
<style scoped>
.my-label {
...
function modUserinfo(){
  axios.post('http://127.0.0.1/my/userinfo', {username:userName.value,user_pic:userPic.value,email:email.value,nickname:nickName.value,id:id.value},{
    headers: {
